...
def create_3d_figure(df):
    fig = px.scatter_3d(
        ...
        size=df["distance"], color=df["distance"],
        opacity=0.8,
        color_continuous_scale=[[0, COLORS['background']], [0.5, COLORS['primary']], [1, COLORS['accent']]],
    )
    ...
    fig.update_layout(
        ...
        scene=dict(
            xaxis=dict(backgroundcolor="rgba(0,0,0,0)", gridcolor=COLORS['primary'], showbackground=False, title_font=dict(color=COLORS['accent'])),
            yaxis=dict(backgroundcolor="rgba(0,0,0,0)", gridcolor=COLORS['primary'], showbackground=False, title_font=dict(color=COLORS['accent'])),
            zaxis=dict(backgroundcolor="rgba(0,0,0,0)", gridcolor=COLORS['primary'], showbackground=False, title_font=dict(color=COLORS['accent'])),
        ),
    )
    return fig

def create_history_areaplot(df: pd.DataFrame):
    df_melted = df.melt(value_vars=["arousal", "valence", "dominance"], id_vars=["step"])
    df_melted = df_melted[df_melted["step"].max() == df_melted["step"]]
    color_map = {"arousal": COLORS['secondary'], "valence": COLORS['accent'], "dominance": COLORS['primary']}
    fig = px.bar(df_melted, x="variable", y="value", color="variable", color_discrete_map=color_map)
    fig.update_layout(
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
        font=dict(color=COLORS['text']), showlegend=False,
        margin=dict(l=30, r=20, b=20, t=10),
        xaxis=dict(title=None, gridcolor='rgba(149, 112, 255, 0.2)'),
        yaxis=dict(title=None, gridcolor='rgba(149, 112, 255, 0.2)', range=[0, 1.1])
    )
    return fig
...
def get_clean_live_df():
    df = (pd.DataFrame(pd.Series(np.arange(11)).rename("x"))
...
global_history_df = pd.DataFrame(data={"arousal": [0], "valence": [0], "dominance": [0], "step": [0]})
    ...
    children=[
        ...
        html.Div(
            ...
            children=[
                ...
                html.Div("Kliknij zdjęcie lub tło, aby zamknąć", style={
                    'marginTop': '15px',
        ...
        # 2. HEADER
        html.Div([
            html.H1("AuraCloud", style={'color': COLORS['accent'], 'margin': 0, 'textAlign': 'center', 'paddingTop': '10px'}),
            html.Div("Real-time Emotion Mapping", style={'textAlign': 'center', 'color': COLORS['secondary'], 'marginBottom': '10px'}),

            # Przycisk DEMO
            html.Button("Test Fullscreen Function", id="btn-trigger-modal", style={
                'background': 'transparent', 'border': f'1px solid {COLORS["secondary"]}',
                'color': COLORS['secondary'], 'padding': '5px 10px', 'borderRadius': '5px', 'cursor': 'pointer',
        ...
        # 3. GŁÓWNY OBSZAR
        html.Div(
            style={'display': 'flex', 'flex': 1, 'padding': '20px', 'gap': '20px', 'height': '85vh', 'boxSizing': 'border-box'},
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div(
                            ...
                            style={'flex': 1, 'position': 'relative', 'display': 'flex', 'justifyContent': 'center', 'alignItems': 'center', 'overflow': 'hidden', 'cursor': 'pointer'},
                            children=[
                                html.Img(src=IMG_SRC, style={'maxWidth': '90%', 'maxHeight': '90%', 'objectFit': 'contain'}),
                                html.Div("⤢", style={'position': 'absolute', 'bottom': '10px', 'right': '10px', 'color': COLORS['accent'], 'fontSize': '24px'})
                            ]
                        )
                    ]
                )
...
def update_metrics(old_3d_fig, n_intervals):
    global global_df, global_vect, global_target, global_history_df, global_now
    ...
    if trigger_id == "interval-component":
        ...
        if np.abs(global_vect - global_target).sum() < 0.3:
            ...
            target_dict = {"arousal": global_target[0], "valence": global_target[1], "dominance": global_target[2], "step": len(global_history_df)}
    ...
    if old_3d_fig and "data" in old_3d_fig and len(old_3d_fig["data"]) > 0:
        ...
        if "layout" not in old_3d_fig: old_3d_fig["layout"] = {}
        old_3d_fig["layout"]["paper_bgcolor"] = 'rgba(0,0,0,0)'
        old_3d_fig["layout"]["plot_bgcolor"] = 'rgba(0,0,0,0)'
    else:
        old_3d_fig = create_3d_figure(global_df)

    return old_3d_fig, create_history_areaplot(global_history_df)
...
def toggle_modal_display(img_trigger_click, overlay_click, modal_img_click, btn_click, store_data, current_style):
    ctx = callback_context
